...
<div class="layuimini-container">
    <div class="layuimini-main">
        ...
        <script type="text/html" id="currentTableBar">
            <a class="layui-btn layui-btn-warm layui-btn-xs" lay-event="finsh">完成维修</a>
        </script>
...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '${basePath}/factory/watch/going/data.json',
            toolbar: '#toolbarDemo',
            ...
            cols: [[
                ...
                {field: 'start_time', width: 200,  title: '开始时间',sort: true,templet:'<div>{{ layui.util.toDateString(d.start_time, "yyyy-MM-dd HH:mm:ss") }}</div>'},
                {width: 150,  title: '已维修天数',sort: true,templet: function (d){
                        //向上取整
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'finsh') {
                layer.confirm('确认完成维修吗', function (index) {
                    $.ajax({
                    	type:"post",
                    	url:"${basePath}/factory/watch/fix/end",
                    	data:{'watchId':data.id},
                    	dataType:"text",
                    	success:function(data){
                    		if(data=="ok"){
                    			layer.alert("维修成功!",function(){
