...
<div class="layui-form layuimini-form">
        ...
        <div class="layui-input-block">
            <select name="regionId" lay-filter="region" lay-verify="required" lay-reqtext="行政区不能为空">
                ...
                <c:forEach items="${regionList}" var="region">
                	<option value="${region.id}" <c:if test="${region.id eq user.regionId}">selected</c:if>>${region.regionName}</option>
                </c:forEach>
            </select>
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <select name="townId" lay-filter="town" id="town">
                ...
                <c:forEach items="${townList}" var="town">
                    <option value="${town.id}" <c:if test="${town.id eq user.townId}">selected</c:if>>${town.townName}</option>
                </c:forEach>
            </select>
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <select name="isolationId" lay-filter="isolation" id="isolation" >
                ...
                <c:forEach items="${isolationList}" var="isolation">
                    <option value="${isolation.id}" <c:if test="${isolation.id eq user.isolationId}">selected</c:if>>${isolation.isolationName}</option>
                </c:forEach>
            </select>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="number" name="tel" placeholder="请输入手机号码" value="${user.tel}" class="layui-input" >
        </div>
    </div>
    <div class="layui-form-item">
        <div class="layui-input-block">
